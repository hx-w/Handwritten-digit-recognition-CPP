...
class Image_helper:
    ...
    def __col_cut(self):
        ...
        for col in range(w):
            ...
            if __is_edge == False:
                if rec == True:
                    self.__smpic.append(
                        self.__pic.crop((front, 0, col + 1, h)))
                    rec = False
            else:
                if rec == False:
                    front = col
    ...
    def save(self):
        ...
        for pic in self.__smpic:
            ...
            print 'Image No.', cnt
            sw, sh = pic.size
            le_rate = 1.0 * (self.__height - 2 * self.__pad) / max(sw, sh)
            pic = pic.resize((int(math.ceil(sw * le_rate)),
                              int(math.ceil(sh * le_rate))))
            
            sw, sh = pic.size
            self.__base = Image.new('RGB',
                                    (self.__height, self.__width)).convert('L')
            FX, FY = self.__width / 2 - sw / 2, self.__height / 2 - sh / 2 - 1
            ...
            self.__base.save(
                self.__path.replace('canvas.png',
                                    'split/' + str(cnt) + '.jpg'))
            # save numbers
